...
// ,DragHandlerFinder
{
	...
	private void updateLocationOfButtonPanel() {
		if (isMinmaxEnabled()) {
			...
			DOM.setStyleAttribute(buttonPanel, "height", buttonPanelHeight
					+ "px");
			DOM.setStyleAttribute(buttonPanel, "marginTop",
					buttonPanelMarginTop + "px");
		}
	}
		...
		if (overBar) {
			...
			if (outerArea != null) {
				...
				if (outerArea.getStyleName().contains(
						"org_semanticsoft_vaaclipse_area"))
	...
	private VAbstractOrderedLayout findOuterArea(Widget w) {
		if (w instanceof VAbstractOrderedLayout) {
			...
			if (styleName != null
					&& !styleName.isEmpty()
					&& (styleName.contains("org_semanticsoft_vaaclipse_area")
							|| styleName
									.contains("org_semanticsoft_vaaclipse_perspective") || styleName
								.contains("org_semanticsoft_vaaclipse_window"))) {
				return bl;
			}
		}
